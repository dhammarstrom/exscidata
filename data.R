...
#'   \item{lac.\*}{lactate values at a specific exercise intensity collected during incremental tests with 5 min steady state ramps, mmol L<sup>-1</sup>}
#'   \item{VO2.\*}{oxygen consumption values at a specific exercise intensity collected during incremental tests with 5 min steady state ramps, ml min<sup>-1</sup>}
#'   \item{VCO2.\*}{carbon dioxide production values at a specific exercise intensity collected during incremental tests with 5 min steady state ramps, ml min<sup>-1</sup>}
#'   \item{VE.\*}{Ventilation during at specific intensity, L min<sup>-1</sup>}
#'   \item{HF.\*}{Heart rate at specific exercise intensity from incremental test, beats min<sup>-1</sup>}
...
#'   \item{RPM.\*}{Cycling cadence from incremental tests, Rounds min<sup>-1</sup>}
...
#'   \item{VO2.max}{Maximal aerobic power (VO<sub>2max</sub>) as recorded during the maximal test, ml min<sup>-1</sup>}
#'   \item{VO2.diff.30sek}{Difference between two consecutive VO<sub>2max</sub> recordings, ml min<sup>-1</sup>}
#'   \item{VO2.diff.30sek.ml_kg}{Difference between two consecutive VO<sub>2max</sub> recordings, ml min<sup>-1</sup> kg<sup>-1</sup>}
#'   \item{RER.max}{Maximal respiratory quotient from the maxial test, VCO<sub>2</sub>/VO<sub>2</sub>}
#'   \item{VE.max}{Maximal ventilation during the maximal test, L min<sup>-1</sup>}
#'   \item{HF.max}{Maximal heart rate during the maximal test, Beats min<sup>-1</sup>}
#'   \item{HF.1min}{Heart rate one minute after the end of the maximal test, Beats min<sup>-1</sup>}
#'   \item{RPE.\*.max}{Maximal rating of percieved excertion, Borg 6-20}
#'   \item{lac.\*min}{Lactate 1 and 14 minutes after the maximal test, mmol L <sup>-1</sup>}
